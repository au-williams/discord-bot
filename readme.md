...
<details>
  ...
  _Every action in Discord can be thought of as an interaction. Clicking buttons, submitting forms, sending messages, etc. When we make buttons to click or forms to submit we must assign them a unique ID that Discord will emit back to us when it's been interacted with. These IDs are set on components and used as keys in `Listeners<object>`._
